...
const GenraSeriesCrimeMovies = () => {
  const navigate = useNavigate();

  // Crime series data
  const crimeSeries = [
    { 
      id: 1, src: assets.dhamar, title: "Dahmer – Monster: The Jeffrey Dahmer Story", rating: "7.9", year: "2022",
      description: "Chronicles Jeffrey Dahmer’s murders, his troubled life, and police failures enabling his crimes.",
      platforms: ["Netflix"],
      trailer: "https://www.youtube.com/watch?v=1PgKjIosF50"
    },
    { 
      id: 2, src: assets.ozark, title: "Ozark", rating: "8.5", year: "2017-2022",
    ...
    { 
      id: 3, src: assets.peakyblinders, title: "Peaky Blinders", rating: "8.8", year: "2013-2022",
  ...
  const platformLogos = {
    ...
    "FX": "https://logos-world.net/wp-content/uploads/2021/08/FX-Logo.png"
  };
    ...
    <div className="relative min-h-screen w-screen text-white overflow-x-hidden">
      ...
      <div className="relative z-10 pt-8 pb-12">
        ...
        <div className="text-center">
          <h1 className="bg-gradient-to-r from-yellow-500 via-orange-500 to-yellow-400 bg-clip-text text-transparent text-6xl md:text-7xl font-bold font-movie-title tracking-wider mb-4">
            CRIME SERIES
          </h1>
          <p className="text-white/80 text-lg md:text-xl font-light tracking-wide">
            Gritty series that dive into the world of crime
          </p>
          
          {/* Decorative line */}
          <div className="mx-auto mt-4 h-1 w-48 bg-gradient-to-r from-yellow-500 to-orange-400 rounded-full"></div>
        </div>
      </div>
      ...
      <div className="relative z-10 px-8 pb-12" id="series-grid">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {crimeSeries.map((series) => (
              <div
                key={series.id}
                className="movie-card group relative bg-gradient-to-br from-black/60 to-gray-900/60 backdrop-blur-md rounded-2xl overflow-hidden border border-yellow-400/20 shadow-2xl hover:shadow-yellow-500/30 hover:scale-110 hover:border-yellow-400/60 transition-all duration-500 hover:backdrop-blur-xl hover:bg-gradient-to-br hover:from-black/80 hover:to-gray-900/80 hover:z-50"
                onMouseEnter={(e) => {
                  // Blur background
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/40">
                    <div className="w-16 h-16 bg-yellow-600/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-2xl">
                  ...
                  <div className="absolute top-4 right-4 bg-yellow-500/90 backdrop-blur-sm px-2 py-1 rounded-full flex items-center gap-1">
                    <img 
                  ...
                  {/* Year Badge */}
                  <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-sm px-3 py-1 rounded-full border border-yellow-400/30">
                    <span className="text-yellow-400 text-sm font-medium">{series.year}</span>
                  </div>
                </div>
                ...
                <div className="p-6 relative">
                  {/* Title */}
                  <h3 className="text-xl font-bold bg-gradient-to-r from-yellow-400 to-orange-300 bg-clip-text text-transparent mb-3 group-hover:from-yellow-300 group-hover:to-orange-200 transition-all duration-300">
                    {series.title}
                  </h3>
                  ...
                  <button 
                    onClick={() => window.open(series.trailer, '_blank')}
                    className="w-full py-2 px-3 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 backdrop-blur-sm border border-yellow-400/50 text-yellow-400 font-medium text-sm rounded-lg hover:from-yellow-500/40 hover:to-orange-500/40 hover:border-yellow-300 hover:text-yellow-200 hover:shadow-lg hover:shadow-yellow-500/20 transition-all duration-300 relative overflow-hidden group-button"
                  >
                  ...
                  <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-gradient-to-r from-yellow-500 via-orange-500 to-yellow-500 blur-2xl -z-10 animate-pulse"></div>
                </div>

                {/* Subtle glow effect */}
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-300 bg-gradient-to-r from-yellow-500 to-orange-500 blur-xl -z-10"></div>
              </div>
            ))}
          </div>
        </div>
...
export default GenraSeriesCrimeMovies;